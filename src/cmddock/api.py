...
def build_app(settings: Settings) -> FastAPI:
    ...
    app = FastAPI(
        title="CmdDock",
        version="0.1.0",
        summary="Local serial command queue daemon",
        lifespan=lifespan,
    )
    ...
    def create_command(payload: CommandCreate, app_state: StateDependency) -> dict:
        record = app_state.database.create_command(payload.command, payload.cwd)
    ...
    def list_commands(
        ...
        return {"commands": app_state.database.list_commands(status)}
